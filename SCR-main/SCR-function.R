...
# VCov: (p,p,G)-array of group-wise coefficients variance-covariance matrices
...
# BIC: Estimated Schwarz Bayesian Information Criterion
# AIC: Estimated Akaike Information Criterion
# HQ: Estimated Hannan-Quin Information Criterion
...
SCR <- function(Y, X, W, Sp, G=5, Phi=1, offset=NULL, fuzzy=F, maxitr=100, delta=1, family="gaussian"){
  ...
  CL <- list()
  for(k in 1:M){
    CL[[k]] <- kmeans(Sp, G, iter.max = 1000)
  ...
  ## Add by Paolo on 19/02/2024
  pp <- length(Beta) 
  if(family=="gaussian"| family=="NB"){
    pp <- pp + G
  }
  BIC <- -2*ML + log(n)*pp
  AIC <- -2*ML + 2*pp
  HQ <- -2*ML + 2*log(log(n))*pp
  
  ## Results
  result <- list(Beta=Beta, Sig=Sig, VCov = VCov, Nu=Nu, group=Ind, sBeta=sBeta, sSig=sSig,
                 ML=ML, itr=k, BIC = BIC, AIC = AIC, HQ = HQ)
  return(result)
}
